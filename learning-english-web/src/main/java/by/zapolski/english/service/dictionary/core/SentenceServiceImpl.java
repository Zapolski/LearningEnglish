...
public class SentenceServiceImpl implements SentenceService {
    ...
    public SentenceInfoDto getSentenceInfo(String sentence) {
        ...
        for (String lemma : sentenceInfo.getLemmas()) {
            if (!"O".equals(lemma)) {
                List<Dictionary> wordList = dictionaryRepository.findByValue(lemma);
                Optional<Dictionary> optional = wordList.stream().min(Comparator.comparingInt(Dictionary::getRank));
